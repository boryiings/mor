...
__device__ float SQDD_fp32_e2m3_fp32(float fp32_value, float scaling_factor) {
    return SQDD<SF_FP32, E8_RZ, 2, 3, false, float>(fp32_value, scaling_factor); 
}
...
__device__ float SQDD_e8_e2m3_fp32(float fp32_value, float scaling_factor) {
    return SQDD<SF_E8, E8_RZ, 2, 3, false, float>(fp32_value, scaling_factor); 
}
...
__device__ float quant_dequant_e8m1(float value) {
    //S1_E8_M23
    uint32_t bits = *reinterpret_cast<uint32_t*>(&value);
    uint32_t sign = (bits >> 31) & 0x1;
    int32_t exponent = (bits >> 23) & 0xFF;
    ...
    {
	if (mantissa <= 32 << 16) mantissa = 0;
	else if (mantissa <= 95 << 16) mantissa = 1 << 22;
	else
	{
	    mantissa = 0;
	    exponent++;
...
__device__ float quant_dequant_e8m2(float value) {
    //S1_E8_M23
    uint32_t bits = *reinterpret_cast<uint32_t*>(&value);
    uint32_t sign = (bits >> 31) & 0x1;
    int32_t exponent = (bits >> 23) & 0xFF;
    ...
    {
	if (mantissa <= 16 << 16) mantissa = 0;
	else if (mantissa <= 47 << 16) mantissa = 1 << 21;
	else if (mantissa <= 80 << 16) mantissa = 2 << 21;
	else if (mantissa <= 111 << 16) mantissa = 3 << 21;
	else {
	    mantissa = 0;
	    exponent++;
	    if (exponent == 255) {
...
__device__ float quant_dequant_e8m3(float value) {
    //S1_E8_M23
    uint32_t bits = *reinterpret_cast<uint32_t*>(&value);
    uint32_t sign = (bits >> 31) & 0x1;
    int32_t exponent = (bits >> 23) & 0xFF;
    ...
    {
	if (mantissa <= 8 << 16) mantissa = 0; 
	else if (mantissa <= 23 << 16) mantissa = 1 << 20; 
	else if (mantissa <= 40 << 16) mantissa = 2 << 20; 
	else if (mantissa <= 55 << 16) mantissa = 3 << 20; 
	else if (mantissa <= 72 << 16) mantissa = 4 << 20; 
	else if (mantissa <= 87 << 16) mantissa = 5 << 20; 
	else if (mantissa <= 104 << 16) mantissa = 6 << 20; 
	else if (mantissa <= 119 << 16) mantissa = 7 << 20; 
	else { 
	    mantissa = 0; 
	    exponent++; 
	    if (exponent == 255) 
...
template <E8MXType T_E8MX_TYPE, ScalingStrategy T_SCALING_STRATEGY>
__device__ float e8mx_scaling_factor(float global_amax) {
    float global_scaling_factor = 1.0; //initialize it to something...
    assert(global_amax > 0);
    if constexpr (T_SCALING_STRATEGY == SS_GLOBAL_AMAX) {
	uint32_t y = *reinterpret_cast<uint32_t*>(&global_amax);
	uint32_t exponent = (y >> 23);
	exponent++;
	assert(exponent < 255); 
	uint32_t y2 = exponent << 23;
	if constexpr(T_E8MX_TYPE == E8MX_E8M1)
	    y2 = y2 | (1 << 22);
	else if constexpr(T_E8MX_TYPE == E8MX_E8M2)
	    y2 = y2 | (3 << 21);
	else if constexpr(T_E8MX_TYPE == E8MX_E8M3)
	    y2 = y2 | (7 << 20);
	float fp_max = *reinterpret_cast<float*>(&y2);
	global_scaling_factor = fp_max / global_amax;
...
__global__ void quantize_e8mx(
    ...
    float global_amax = global_amax_device[0];
    float scaling_factor = e8mx_scaling_factor<T_E8MX_TYPE, T_SCALING_STRATEGY>(global_amax);
	...
	for (int c = 0; c < block_num_cols; c += blockDim.x)
	{
	    ...
	    fp32_value1 *= scaling_factor;
	    if (fp32_value1 != 0) {
		if constexpr (T_E8MX_TYPE == E8MX_E8M1)
		    dequantized_value1 = quant_dequant_e8m1(fp32_value1);
		else if constexpr (T_E8MX_TYPE == E8MX_E8M2)
		    dequantized_value1 = quant_dequant_e8m2(fp32_value1);
		else if constexpr (T_E8MX_TYPE == E8MX_E8M3)
		    dequantized_value1 = quant_dequant_e8m3(fp32_value1);
	    }
	    dequantized_value1 /= scaling_factor;

	    fp32_value2 *= scaling_factor;
	    if (fp32_value2 != 0) {
		if constexpr (T_E8MX_TYPE == E8MX_E8M1)
		    dequantized_value2 = quant_dequant_e8m1(fp32_value2);
		else if constexpr (T_E8MX_TYPE == E8MX_E8M2)
		    dequantized_value2 = quant_dequant_e8m2(fp32_value2);
		else if constexpr (T_E8MX_TYPE == E8MX_E8M3)
		    dequantized_value2 = quant_dequant_e8m3(fp32_value2);
	    }
	    dequantized_value2 /= scaling_factor;
...
__global__ void block_amax_and_non_zero_channel(
    const uint32_t* input_device,
    float* block_amax_device,
    int* block_non_zero_device,
    int num_rows,
    ...
    int block_num_rows,
    int block_num_cols,
    int number_of_sub_tensors_per_block) {
    int threads_x = blockDim.x;
    int threads_y = blockDim.y;
    int threads_per_block = threads_x * threads_y;

    int start_index_x = (num_cols / gridDim.x) * blockIdx.x;
    int   end_index_x = start_index_x + (num_cols / gridDim.x);

    int start_index_y = (num_rows / gridDim.y) * blockIdx.y;
    int   end_index_y = start_index_y + (num_rows / gridDim.y);

    int N_packed = num_cols/2;
    start_index_x /= 2;
      end_index_x /= 2;
    ...
    for(int r = start_index_y; r < end_index_y; r += blockDim.y)
    {
        ...
        for(int c = start_index_x; c < end_index_x; c += blockDim.x)
        {
            ...
            uint32_t value = input_device[row * N_packed + col];
	...
	blockReduceAmaxNonZero(local_amax, local_nnz, s_amax, s_nnz, tid);
        ...
        if (tid == 0)
        {
            ...
            block_non_zero_device[sub_tensor_id] = s_nnz[0];
        }
    }
    else
    {
        ...
        if (block_num_cols == 1)
        {
            if (ty == 0)
            {
                block_amax_device[starting_sub_tensor_id + tx] = local_amax1;
                block_amax_device[starting_sub_tensor_id + blockDim.x + tx] = local_amax2;

                block_non_zero_device[starting_sub_tensor_id + tx] = local_nnz1;
                block_non_zero_device[starting_sub_tensor_id + blockDim.x + tx] = local_nnz2;
	    }
        }
        else //We in threadx.x each block_num_cols/2 belongs to 1 block and we need to reduce across that...
        {
            assert(block_num_rows == 1); assert(threads_y == 1); assert(ty == 0);
            ...
            int final_nnz = 0;
            int group_size = block_num_cols/2;

            reduce_amax_nnz_per_group(local_amax, local_nnz, final_amax, final_nnz, group_size);
            if ((tx % group_size) == 0)
            {
                int group_id = tx / group_size;
                block_amax_device[starting_sub_tensor_id + group_id] = final_amax;
                block_non_zero_device[starting_sub_tensor_id + group_id] = final_nnz;
            }
        }
    }
}
...
template <ScalingStrategy T_SCALING_STRATEGY>
__global__ void block_e4m3_relative_error_channel(
    const uint32_t* input_device,
    uint32_t* output_device,
    float* block_amax_device,
    float* block_error_device,
    ...
    float global_amax = global_amax_device[0];
    if (number_of_sub_tensors_per_block == 1) {
	float block_amax = block_amax_device[starting_sub_tensor_id];
	scaling_factor1 = get_scaling_factor<T_SCALING_STRATEGY>(global_amax, block_amax);
    ...
    else
    {
        ...
        if (block_num_cols == 1) {
	    float block_amax = block_amax_device[starting_sub_tensor_id + tx];
	    scaling_factor1 = get_scaling_factor<T_SCALING_STRATEGY>(global_amax, block_amax);

	    block_amax = block_amax_device[starting_sub_tensor_id + blockDim.x + tx];
	    scaling_factor2 = get_scaling_factor<T_SCALING_STRATEGY>(global_amax, block_amax);
        }
        else
        {
            int threads_y = blockDim.y;
	    ...
	    float block_amax = block_amax_device[starting_sub_tensor_id + group_id];
	    scaling_factor1 = get_scaling_factor<T_SCALING_STRATEGY>(global_amax, block_amax);
            scaling_factor2 = scaling_factor1;
        }
    }

    float local_error1 = 0.0;
    float local_error2 = 0.0;
    ...
    for(int r = start_index_y; r < end_index_y; r += blockDim.y)
    {
        ...
        for(int c = start_index_x; c < end_index_x; c += blockDim.x)
        {
            ...
            {
		if constexpr (T_SCALING_STRATEGY == SF_E8) {
		    dequantized_value1 = SQDD_e8_e2m3_fp32(fp32_value1, scaling_factor1);
		} else {
		    dequantized_value1 = SQDD_fp32_e2m3_fp32(fp32_value1, scaling_factor1);
		}
		float value_error = fabsf((dequantized_value1 - fp32_value1) / fp32_value1);
		local_error1 += value_error;
            }

            if (fp32_value2 != 0)
            {
		if constexpr (T_SCALING_STRATEGY == SF_E8) {
		    dequantized_value2 = SQDD_e8_e2m3_fp32(fp32_value2, scaling_factor2);
		} else {
		    dequantized_value2 = SQDD_fp32_e2m3_fp32(fp32_value2, scaling_factor2);
		}
		float value_error = fabsf((dequantized_value2 - fp32_value2) / fp32_value2);
                local_error2 += value_error;
            }

            uint32_t pruned_bf16_value1 = fp32_to_bf16(dequantized_value1);
            uint32_t pruned_bf16_value2 = fp32_to_bf16(dequantized_value2);
            uint32_t merged_bf16 = pruned_bf16_value1 << 16 | pruned_bf16_value2;
            output_device[row * N_packed + col] = merged_bf16;
        }
    }

    extern __shared__ float sdata[];

    starting_sub_tensor_id = block_id * number_of_sub_tensors_per_block;
    int tid = threadIdx.y * blockDim.x + threadIdx.x;
    float local_error = local_error1 + local_error2;
    blockReduceError(local_error, sdata, tid);
    if (tid == 0) block_error_device[starting_sub_tensor_id + 0] = sdata[0];
    else if (tid < number_of_sub_tensors_per_block) block_error_device[starting_sub_tensor_id + tid] = 0.0;
}
...
at::Tensor fake_quantize_e8mx(const at::Tensor& input, int64_t num_rows, int64_t num_cols, int64_t block_num_rows, int64_t block_num_cols, int64_t quantize_type, int64_t scaling_strategy) {
    // Sanity checks for the input 2D tensor.
    TORCH_CHECK(input.dtype() == at::kBFloat16);
    TORCH_CHECK(input.sizes()[0] == num_rows);
    TORCH_CHECK(input.sizes()[1] == num_cols);
	...
	torch::TensorOptions().dtype(torch::kFloat).device(input.device());
    // Create the global amax scalar.
    auto global_amax = torch::empty({1}, float_options);
    TORCH_INTERNAL_ASSERT(global_amax.device().type() == at::DeviceType::CUDA);
    float* global_amax_ptr = global_amax.data_ptr<float>();
    // Create the global non zero scalar.
    auto global_non_zero = torch::empty({1}, int_options);
    TORCH_INTERNAL_ASSERT(global_non_zero.device().type() == at::DeviceType::CUDA);
    int32_t* global_non_zero_ptr = global_non_zero.data_ptr<int32_t>();
    ...
    block_amax_and_non_zero<<<gridDim, blockDim, sharedMemSize>>>(input_uint32_ptr, block_amax_ptr, block_non_zero_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
    global_amax_and_non_zero<<<reductionGridDim, reductionBlockDim, sharedMemSize>>>(block_amax_ptr, block_non_zero_ptr, global_amax_ptr, global_non_zero_ptr, num_blocks);
    E8MXType e8mx_type = static_cast<E8MXType>(quantize_type);
    ScalingStrategy ss_type = static_cast<ScalingStrategy>(scaling_strategy);
    if (e8mx_type == E8MX_E8M1)
	if (scaling_strategy == SS_GLOBAL_AMAX)
	    quantize_e8mx<E8MX_E8M1, SS_GLOBAL_AMAX><<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_amax_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
	else
	    quantize_e8mx<E8MX_E8M1, SS_E8><<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_amax_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
    else if (e8mx_type == E8MX_E8M2)
	if (scaling_strategy == SS_GLOBAL_AMAX)
	    quantize_e8mx<E8MX_E8M2, SS_GLOBAL_AMAX><<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_amax_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
	else
	    quantize_e8mx<E8MX_E8M2, SS_E8><<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_amax_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
    else if (e8mx_type == E8MX_E8M3)
	if (scaling_strategy == SS_GLOBAL_AMAX)
	    quantize_e8mx<E8MX_E8M3, SS_GLOBAL_AMAX><<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_amax_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
	else
	    quantize_e8mx<E8MX_E8M3, SS_E8><<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_amax_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed);
...
// Given an input 2D tensor with bf16 type, we will use MoR to quantize each block, and then cast back to bf16.
// We assume that the input tensor is row major, so the elements in one row are contiguous.
void fake_quantize_e8mx_inplace(const at::Tensor& input, int64_t num_rows, int64_t num_cols, int64_t block_num_rows, int64_t block_num_cols, int64_t quantize_type, int64_t scaling_strategy, at::Tensor& result) {
    // Sanity checks for the input 2D tensor.
    TORCH_CHECK(input.dtype() == at::kBFloat16);
    TORCH_CHECK(input.sizes()[0] == num_rows);
    TORCH_CHECK(input.sizes()[1] == num_cols);
...
int get_number_of_sub_tensors_per_block(int threads_x, int block_num_rows, int block_num_cols) {
    int number_of_sub_tensors_per_block = 1;
    int num_values_in_one_row = 2 * threads_x;
    if (block_num_cols == 1)
	number_of_sub_tensors_per_block = num_values_in_one_row;
    else if (block_num_cols < num_values_in_one_row)
	number_of_sub_tensors_per_block = num_values_in_one_row / block_num_cols;

    if (block_num_rows == 1) {
        if (block_num_cols < num_values_in_one_row)
	    assert((num_values_in_one_row % block_num_cols) == 0);
        else
	    assert(block_num_cols % num_values_in_one_row == 0);
        if (number_of_sub_tensors_per_block > 1)
	    assert((block_num_cols / 2) <= 32); //we want it to fit within a warp if more than //1 block in X direction...
    }
    return number_of_sub_tensors_per_block;
}

int get_shared_memory_size(int threads_per_block, int number_of_sub_tensors_per_block, int reduction_threads_per_block) {
    int sharedMemSize = 0;
    if (number_of_sub_tensors_per_block == 1) {
        assert((threads_per_block % 32) == 0);
        int warps_per_block = threads_per_block / 32;
	...
	sharedMemSize = 2 * threads_per_block * sizeof(float);
    }
    int warps_in_reduction = reduction_threads_per_block / 32;
    int reduction_size = warps_in_reduction * sizeof(float) * 2;
    return max(sharedMemSize, reduction_size);
}

template <ScalingStrategy T_SCALING_STRATEGY>
std::vector<at::Tensor> fake_quantize_channel_scaling_template(const at::Tensor& input, int64_t num_rows, int64_t num_cols, int64_t block_num_rows, int64_t block_num_cols, double e4m3_threshold) {
    // Sanity checks for the input 2D tensor.
    TORCH_CHECK(input.dtype() == at::kBFloat16);
    TORCH_CHECK(input.sizes()[0] == num_rows);
    TORCH_CHECK(input.sizes()[1] == num_cols);
    ...
    assert(num_rows % block_num_rows == 0);
    assert(num_cols % block_num_cols == 0);
    int row_num_blocks = num_rows / block_num_rows;
    int col_num_blocks = num_cols / block_num_cols;
    int num_blocks = col_num_blocks * row_num_blocks;
    ...
    int threads_x = 32;
    int threads_y = 32;
    assert(block_num_rows == 1 || block_num_cols == 1);
    if (block_num_rows == 1) { threads_x = 64;  threads_y = 1;}
    if (block_num_cols == 1) { threads_x = 32;  threads_y = 16;} //We //assume we will have atleast 16 values in each block in Y axis
    int threads_per_block = threads_x * threads_y;

    int number_of_sub_tensors_per_block = get_number_of_sub_tensors_per_block(threads_x, block_num_rows, block_num_cols);
    int grid_size_x = max(col_num_blocks / number_of_sub_tensors_per_block, 1);
    int grid_size_y = row_num_blocks;

    int threads_x_reduction = 1024;
    dim3 blockDim(threads_x, threads_y);  // 256 threads per block
    dim3 gridDim(grid_size_x, grid_size_y);
    dim3 reductionBlockDim(threads_x_reduction, 1);  // 256 threads per block
    dim3 reductionGridDim(1, 1);

    int sharedMemSize = get_shared_memory_size(threads_per_block, number_of_sub_tensors_per_block, threads_x_reduction);

    float e4m3_threshold_fp32 = static_cast<float>(e4m3_threshold);
    block_amax_and_non_zero_channel<<<gridDim, blockDim, sharedMemSize>>>(input_uint32_ptr, block_amax_ptr, block_non_zero_ptr, num_rows, num_cols, block_num_rows, block_num_cols, number_of_sub_tensors_per_block);
    global_amax_and_non_zero<<<reductionGridDim, reductionBlockDim, sharedMemSize>>>(block_amax_ptr, block_non_zero_ptr, global_amax_ptr, global_non_zero_ptr, num_blocks);
    block_e4m3_relative_error_channel<T_SCALING_STRATEGY><<<gridDim, blockDim, sharedMemSize>>>(input_uint32_ptr, result_uint32_ptr, block_amax_ptr, block_error_ptr, global_amax_ptr, num_rows, num_cols, block_num_rows, block_num_cols, number_of_sub_tensors_per_block);
    // cudaDeviceSynchronize();
    global_e4m3_relative_error<<<reductionGridDim, reductionBlockDim, sharedMemSize>>>(block_error_ptr, global_error_ptr, num_blocks);
    maybe_use_bf16<<<gridDim, blockDim>>>(input_uint32_ptr, result_uint32_ptr, global_non_zero_ptr, global_error_ptr, meta_ptr, row_packed, col_packed, block_num_rows_packed, block_num_cols_packed, e4m3_threshold_fp32);
...
std::vector<at::Tensor> fake_quantize_channel_scaling(const at::Tensor& input, int64_t num_rows, int64_t num_cols, int64_t block_num_rows, int64_t block_num_cols, double e4m3_threshold, int64_t ss_mode) {
    ScalingStrategy scaling_strategy = static_cast<ScalingStrategy>(ss_mode);
    switch (scaling_strategy) {
	case SS_GLOBAL_AMAX:
	    return fake_quantize_channel_scaling_template<SS_GLOBAL_AMAX>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
	case SS_BLOCK_AMAX:
	    return fake_quantize_channel_scaling_template<SS_BLOCK_AMAX>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
	case SS_E8:
	    return fake_quantize_channel_scaling_template<SS_E8>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
	case SS_CURRENT_SCALING:
	    return fake_quantize_channel_scaling_template<SS_CURRENT_SCALING>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
    }
}

template <ScalingStrategy T_SCALING_STRATEGY>
std::vector<at::Tensor> inplace_fake_quantize_channel_scaling_template(at::Tensor& input, int64_t num_rows, int64_t num_cols, int64_t block_num_rows, int64_t block_num_cols, double e4m3_threshold) {
    // Sanity checks for the input 2D tensor.
    TORCH_CHECK(input.dtype() == at::kBFloat16);
    TORCH_CHECK(input.sizes()[0] == num_rows);
    TORCH_CHECK(input.sizes()[1] == num_cols);
...
std::vector<at::Tensor> inplace_fake_quantize_channel_scaling(at::Tensor& input, int64_t num_rows, int64_t num_cols, int64_t block_num_rows, int64_t block_num_cols, double e4m3_threshold, int64_t ss_mode) {
    ScalingStrategy scaling_strategy = static_cast<ScalingStrategy>(ss_mode);
    switch (scaling_strategy) {
	case SS_GLOBAL_AMAX:
	    return inplace_fake_quantize_channel_scaling_template<SS_GLOBAL_AMAX>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
	case SS_BLOCK_AMAX:
	    return inplace_fake_quantize_channel_scaling_template<SS_BLOCK_AMAX>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
	case SS_E8:
	    return inplace_fake_quantize_channel_scaling_template<SS_E8>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
	case SS_CURRENT_SCALING:
	    return inplace_fake_quantize_channel_scaling_template<SS_CURRENT_SCALING>(input, num_rows, num_cols, block_num_rows, block_num_cols, e4m3_threshold);
	    break;
    }
}
...
TORCH_LIBRARY_IMPL(mor, CUDA, m) {
  ...
  m.impl("fake_quantize_e8mx", &fake_quantize_e8mx);
  m.impl("fake_quantize_e8mx_inplace", &fake_quantize_e8mx_inplace);
  m.impl("fake_quantize_channel_scaling", &fake_quantize_channel_scaling);
  m.impl("fake_quantize_channel_scaling_inplace", &inplace_fake_quantize_channel_scaling);
}
