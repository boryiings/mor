...
# Compile GPU computation file with optimization
echo "Compiling GPU computation file (gpu_computation.cu) with optimizations..."
nvcc -O3 -arch=sm_80 --use_fast_math -c gpu_computation.cu -o gpu_computation.o
##nvcc -O3 -arch=sm_86 --use_fast_math -Xptxas -dlcm=ca -maxrregcount=32 -c gpu_computation.cu -o gpu_computation.o
if [ $? -ne 0 ]; then
    echo "Error: Compilation of gpu_computation.cu failed!"
    exit 1
fi
echo "Successfully compiled gpu_computation.cu to gpu_computation.o with optimizations."
